...
/**
 * {@linkplain BCryptPassword} is a {@link Password} implementation that uses
 * BCrypt's algorithm internally. It encapsulates JBCrypt's implementation by
 * storing the hashed password as an attribute, and only exposes publicly the
 * interface's {@link #verify(String)} method, for checking if a plaintext
 * password matches with the hashed one.
 */
public final class BCryptPassword implements Password {
